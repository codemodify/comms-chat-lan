...
// The beacon: "there is a server here", over UDP multicast.
//
// One Beacon type serves both roles. The server calls [Beacon.Update] with
// what it wants said and Run repeats it; a client calls Run without ever
// calling Update, and only listens. A Beacon with nothing to announce
// sends nothing, which is exactly what a client should do.
//
...
// machine. What this app needs is one sentence, repeated: "I am <id>,
// connect to me on <port>". That is what the beacon sends.
//
// The cost is stated in the README and in docs/protocol.md: the server is
// discoverable only by this application's own clients. `avahi-browse` will
// not show it. Where multicast does not reach — a client on another
// segment, a VPN, a container network without a multicast route — the
// client is told the address instead, with -server or UITK_CHAT_SERVER.
const (
	...
	// to the JSON parser. It carries the protocol version, so a client of
	// the peer-to-peer version of this application and a server of this
	// one never even parse each other's packets.
	BeaconMagic = "CHATLAN/2 "
	...
	// BeaconInterval is how often the server re-announces itself.
	BeaconInterval = 12 * time.Second
	// BeaconExpiry is how long a server stays found without being heard
	// from. Three missed announcements: one lost packet, which on a busy
	// wireless network is routine, must not make the server blink out.
...
type Beacon struct {
	...
	// Iface pins the multicast interface by name. Empty means every
...
// NewBeacon is the beacon configured from the environment:
// UITK_CHAT_IFACE pins the interface. A server calls [Beacon.Update] on
// it; a client leaves it silent and only listens.
func NewBeacon() *Beacon {
	return &Beacon{Iface: strings.TrimSpace(os.Getenv(chat.EnvIface)), Loopback: true}
}
...
// if there is anything to announce, reads announcements and expires
// servers that go quiet, until ctx is cancelled. It sends a goodbye on the
// way out.
...
func (b *Beacon) multicastInterfaces(ev DiscoveryEvents) []net.Interface {
	...
	if len(out) == 0 && b.Iface != "" {
		ev.note("beacon: " + chat.EnvIface + "=" + b.Iface + " matches no up multicast interface")
	}
	return out
}
...
func encodeBeacon(a Announcement) ([]byte, error) {
	...
	if len(pkt) > beaconMaxPacket {
		// Nothing in an announcement can grow without bound any more —
		// it is an id, a name and a port — so this is a programming
		// error rather than a packet to trim and send anyway.
		return nil, fmt.Errorf("chat: announcement too large (%d bytes)", len(pkt))
	}
	return pkt, nil
}

// parseBeacon decodes a packet. The server's address is taken from the
// packet source, never from the payload: an announcement can lie about
// what it is called, but it cannot point a client at a machine it does not
// occupy.
