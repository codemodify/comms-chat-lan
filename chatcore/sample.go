...
func SeedSample(s *Store) {
	...
	s.PutPeer(Peer{
		...
		Presence: PresenceOnline, LastSeen: time.Now(), Known: true,
	})
	s.PutPeer(Peer{
		ID: theo, Nick: "theo", Color: ColorForID(theo), Host: "workshop",
		Addr: "192.168.1.31:47772", Rooms: []string{"general"},
		Presence: PresenceAway, LastSeen: time.Now().Add(-4 * time.Minute), Known: true,
	})
...
// StartSample runs a whole daemon in this process over a seeded store, on
// a private socket, with discovery off. The screenshot tool and the
// headless UI tests use it; nothing else should.
func StartSample(ctx context.Context) (socket string, stop func(), err error) {
	return startSeeded(ctx, SeedSample)
}
