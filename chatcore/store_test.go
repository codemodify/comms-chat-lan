...
func TestStoreOrdersMessagesByTheSameRuleEverywhere(t *testing.T) {
	tempHome(t)
	s := NewMemoryStore()
	base := time.Date(2026, 9, 23, 10, 0, 0, 0, time.UTC)
	conv := RoomConv("general")

	// Deliberately out of order, and with one tie on Sent that only the
	// sequence number can break.
	in := []Message{
		{ID: "c", Conv: conv, Sent: base.Add(2 * time.Second), Seq: 3},
		{ID: "a", Conv: conv, Sent: base, Seq: 1},
		{ID: "b2", Conv: conv, Sent: base.Add(time.Second), Seq: 5},
		{ID: "b1", Conv: conv, Sent: base.Add(time.Second), Seq: 2},
	}
	for _, m := range in {
