...
// Client is the front ends' side of the clientd socket. Both the GUI and
...
//
// A Client reconnects by itself. comms-chat-lan-clientd being restarted
// under a running UI is an ordinary event — an upgrade, a crash, a
// `systemctl --user restart` — and a front end that has to be restarted
// with it is a front end nobody leaves open.
type Client struct {
	Socket string
...
// Dial connects to comms-chat-lan-clientd.
...
// DialWait retries until clientd appears or wait elapses. It is what the
// front ends use at startup, so starting the daemon and the UI in the
// same breath works.
...
func (c *Client) connect() error {
	...
	if err != nil {
		return fmt.Errorf("chat: dial %s: %w (is comms-chat-lan-clientd running?)", c.Socket, err)
	}
	c.mu.Lock()
	c.conn = conn
...
// OnEvent registers the handler for clientd's events. It is called from the
...
func (c *Client) dropped(conn net.Conn) {
	...
	for _, ch := range pending {
		select {
		case ch <- Response{Error: &RPCError{Code: ErrClosed, Message: "comms-chat-lan-clientd: the connection closed"}}:
		default:
		}
	}
	if c.closed.Load() {
...
// is local and fast: clientd answers from its cache and never blocks a
// reply on the server, so a slow call means something is wrong rather than
// something is busy.
func callTimeout(method string) time.Duration {
	switch method {
	case MethodFilesOffer:
		// clientd hashes the file before it offers it, and that is
...
func (c *Client) call(method string, params any, out any) error {
	...
	if c.conn == nil {
		c.mu.Unlock()
		return fmt.Errorf("chat: not connected to comms-chat-lan-clientd")
	}
	c.pending[id] = ch
...
// Status is everything a front end needs for its status bar, including
// whether clientd is talking to a server right now.
...
// Send composes a message. It returns once clientd has stored it locally,
// whether or not the server is reachable; the server's acceptance arrives
// later as a chat.state event.
...
// OfferFile offers a local file to a peer, through the server. The path
// is opened by clientd, which runs as the same user as the front end —
// the socket makes sure of that — so this is not a way to read somebody
// else's files.
